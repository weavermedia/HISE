...
{
	NodeUIUpdaterTests() : UnitTest("NodeUIUpdater Tests", "UI") {}
	...
	struct TestAsyncUpdater : public NodeUIAsyncUpdater::WithData<TestData>
	{
		...
		void onUIAsyncUpdate() override { if (onCallback) onCallback(); }
	};

	struct TestTimer : public NodeUITimer::WithData<TestData>
	{
		std::function<void()> onCallback;
		void onUITimerCallback() override { if (onCallback) onCallback(); }
	};
	...
	template<typename F> struct AudioThreadT { F callback; };
	template<typename F> struct UIThreadT { F callback; };
	template<typename F> struct StopWhenT { F condition; };

	// Helper functions to create wrapper types (MSVC doesn't support in-class CTAD)
	template<typename F> static AudioThreadT<F> AudioThread(F&& f) { return { std::forward<F>(f) }; }
	template<typename F> static UIThreadT<F> UIThread(F&& f) { return { std::forward<F>(f) }; }
	template<typename F> static StopWhenT<F> StopWhen(F&& f) { return { std::forward<F>(f) }; }
	...
	void testScenario6FixWithScopedAllVoiceSetter()
	{
		...
		for (int i = 0; i < NUM_POLYPHONIC_VOICES; i++)
		{
			expectEquals(polyData.getWithIndex(i), 42,
				"Voice " + String(i) + " should be updated to 42");
		}
	}
