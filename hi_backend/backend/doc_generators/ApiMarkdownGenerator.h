...
namespace ScriptingApiDatabase
{
	...
	struct Resolver : public hise::MarkdownParser::LinkResolver
	{
		Resolver(File scriptingDocRoot) :
			...
			apiDump(new DynamicObject()),
			exampleDump(new DynamicObject())
		{}
		...
		{
			auto ragFile = docRoot.getChildFile("rag");
			auto json = JSON::toString(var(apiDump.get()));
			ragFile.getChildFile("scripting_api.json").replaceWithText(json);

			auto json2 = JSON::toString(var(exampleDump.get()));
			ragFile.getChildFile("code_examples.json").replaceWithText(json2);	
		}
		...
		DynamicObject::Ptr apiDump, exampleDump;
