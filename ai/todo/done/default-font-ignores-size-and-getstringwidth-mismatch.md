...
So `g.setFont("Default", 30)` and `g.setFontWithSpacing("Default", 30, x)` (`ScriptingGraphics.cpp` 2102 / 2113) silently draw at 13 px. Knock-on: `setExtraKerningFactor` scales by the *actual* height, so letter-spacing is `spacing x 13`, not `spacing x 30`. No error, no warning. Every other caller of `getFontFromString` (label wrappers, keyboard octave text, markdown style data, `Content.createPath`... ~12 sites) inherits the same behaviour for `"Default"`.
...
4. **Off-by-one out-of-bounds read**: `characterWidths[jlimit<uint8>(31, 128, c)]` on a `float characterWidths[128]` - any char >= 128 (every non-ASCII glyph, e.g. the degree sign, the en-dash, `...`) reads index 128, one past the end. Should clamp to 127 (and 32..126 is what the ctor fills; index 31 and 127 are 0).
...
- After Bug 1 fix: text renders at 30 px; letter-spacing gap is `spacing x 30`; the `+nudge` row (rect shifted by `spacing x 30 / 2`) is centred.
...
The motivating symptom (centred text with `setFontWithSpacing` sitting `spacing x height / 2` px left of centre) is plain JUCE behaviour - `Font::getGlyphPositions` adds the extra kerning after every glyph including the last, and `Justification::centred` centres the advance box. Not a HISE bug; noted here only because Bugs 1 and 2 made it impossible to correct from script with `"Default"`.
