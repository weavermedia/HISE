...
**Status:** DONE. Committed on meatbeats as `3c8f111ae` (2026-07-14), both parts: the attribute setter writes lastSaturationModValue, and handlePeakDisplay() only mirrors the chain output while voices are active. Verified by Dan in Sublime (Shape knob morphs the source waveform graph live with no note held). Fork-only: rides along with the modulatable-Saturation feature `29dc7eb00` if that ever goes upstream. Moved to done 2026-09-07.
