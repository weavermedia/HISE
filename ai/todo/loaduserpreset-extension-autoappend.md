...
Caveat: `File::withFileExtension` **replaces** everything after the last dot, so a preset named `v1.2 Lead` would become `v1.preset`. Preset names with dots are legal, so the robust form is a plain string append:
